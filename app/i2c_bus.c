...
{
  I2C_HandleTypeDef*    hi2c = get_i2c_handle(addr);

  if(HAL_I2C_Master_Transmit(hi2c, (addr << 1), data, len, I2C_DEFAULT_TIMEOUT) != HAL_OK)
  {
    return false;
  }
  return true;
    ...
    uint8_t* rdata, uint8_t rdata_len)
{
  ...
  if(HAL_I2C_Master_Transmit(hi2c, (addr << 1), wdata, wdata_len, I2C_DEFAULT_TIMEOUT) != HAL_OK)
  {
    return false;
  }
...
i2c_bus_read_sync(I2CBusNumber bus, I2CSlaveAddress addr, uint8_t* data, uint16_t len)
{
  ...
  if(HAL_I2C_Master_Receive(hi2c, (addr << 1), data, len, I2C_DEFAULT_TIMEOUT) != HAL_OK)
  {
    return false;
  }
  return true;
